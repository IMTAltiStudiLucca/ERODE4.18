 ...
 * 
 * Andrea Vandin: adapted from https://github.com/tudo-aqua/z3-turnkey/blob/7faae0ffc87941bdde439a4e64a6382a719a8b8a/src/main/java/com/microsoft/z3/Z3Loader.java#L55
 */
...
/**
 * Handles automatic unpacking and loading for the Z3 native libraries. The Z3 code is rewritten to invoke this class
 * automatically. It should not be called manually.
 */
public final class Z3Loader {
     ...
     * </ol>
     * Loading is performed during the first Z3 operation that uses native code. A loading error will be thrown during
     * the same operation; otherwise, Z3 should be in a usable state.
     *
     ...
     * @throws IllegalStateException if the Z3 library distribution is incomplete, indicating a packaging error.
     */
    public static void loadZ3() {
        ...
        //final InputStream libZ3 = clazz.getResourceAsStream(getLibraryPath(os, cpu, "z3").substring(1));
        //final InputStream libZ3Java = clazz.getResourceAsStream(getLibraryPath(os, cpu, "z3java").substring(1));
        InputStream libZ3=null;
        InputStream libZ3Java=null;
		try {
			libZ3 = new FileInputStream(new File(getLibraryPath(os, /*cpu*/null, "z3").substring(1)));
			libZ3Java = new FileInputStream(new File(getLibraryPath(os, /*cpu*/null, "z3java").substring(1)));
		} catch (FileNotFoundException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
        
        assertLibrariesFound(libZ3, libZ3Java, os, /*cpu*/null);
        ...
        try {
        	...
        	Path libraryDir = Paths.get("z3-turnkey/");
        	
        	if(!libraryDir.toFile().exists()) {
            ...
            if(!libZ3Out.toFile().exists()) {
            	unpackLibrary(libZ3, libZ3Out);
            }
            if(!libZ3JavaOut.toFile().exists()) {
...
//        }
    ...
    private static String getLibraryPath(final OperatingSystem os, final CPUArchitecture cpu, final String library) {
    	String a ="/native/" + os.name;
    	if(cpu!=null) {
    		a =a+ "-" + cpu.name + "/" + getLibraryName(os, library);
    	}
    	else {
    		a =a+ "/" + getLibraryName(os, library);
    	}
    	//a ="/native/" + os.name + "-" + cpu.name + "/" + getLibraryName(os, library);
        return a;
    }
     ...
     */
    private static void assertLibrariesFound(final InputStream libZ3, final InputStream libZ3Java,
        ...
        if (libZ3 == null && libZ3Java == null) {
        	if(cpu!=null) {
        		...
        		throw new UnsatisfiedLinkError("No native libraries present for " + os);
        	}
