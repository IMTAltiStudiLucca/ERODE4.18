...
public class CRNBisimulationsNAryWithoutListsOfReactions {

	public static final RoundingMode RM = RoundingMode.HALF_DOWN;
...
//		}
		...
		refine(red,crn,obtainedPartition,/*multisetCoefficients,*/speciesCounters,terminator,out,bwOut);
...
//		}
		...
		if(red.equals(Reduction.FE)) {
			...
			computeMultisetCoefficients(crn.getReactions(), terminator, multisetCoefficients, crn.getMaxArity());
		}
		...
		while(splittersGenerator.hasSplittersToConsider()){
			...
			split(red,crn,partition,splittersGenerator,multisetCoefficients,iteration,consideredAtIteration,speciesCounters,speciesCountersHM/*,reactionsToConsiderForEachSpecies*/);
	...
	*/
	private static void computeMultisetCoefficients(Collection<ICRNReaction> reactions, Terminator terminator,
...
//			}
	...
	private static void split(Reduction red, ICRN crn, IPartition partition, SplittersGenerator splittersGenerator, HashMap<IComposite, BigDecimal> multisetCoefficients, Integer iteration, HashMap<ICRNReaction,Integer> consideredAtIteration, ISpeciesCounterHandler[] speciesCounters, HashMap<ISpecies, ISpeciesCounterHandler> speciesCountersHM/*,ArrayListOfReactions[] reactionsToConsiderForEachSpecies*/) {
		...
		anyReactionFound = computeMeasuresUsedToSplitWithRespectToTheSplitter(red,crn,  blockSPL,splitterGenerators,partition,multisetCoefficients,iteration,consideredAtIteration,speciesCounters,speciesCountersHM/*,reactionsToConsiderForEachSpecies*/);
		...
		if(speciesCounters!=null) {
			CRNBisimulationsNAryWithoutListsOfReactions.initializeAllCounters(splitterGenerators,speciesCounters);
		}
		else {
			//....I do it in split
			//speciesCountersHM.
	...
	private static boolean partitionBlocksOfGivenSpecies(IPartition partition, HashSet<ISpecies> consideredSpecies,IBlock blockToCheckIfSplit, HashSet<IBlock> splittedBlocks, SpeciesCounterField keyField, ISpeciesCounterHandler[] speciesCounters, HashMap<ISpecies, ISpeciesCounterHandler> speciesCountersHM) {
		...
		for (ISpecies splitterGenerator : consideredSpecies) {
			...
			ISpeciesCounterHandler counters =getOrAddSpeciesCounterHandler(speciesCounters, speciesCountersHM, splitterGenerator,false);
			if(counters==null) {
				continue;
			}
...
//			ISpeciesCounterHandler counters = speciesCounters[splitterGenerator.getID()];
			...
			if(keyField.equals(SpeciesCounterField.FRVECTOR)){
				...
				if(CRNBisimulationsNAryWithoutListsOfReactions.USETOLERANCE){
					block.getBST(CRNBisimulationsNAryWithoutListsOfReactions.TOLERANCE).put(counters.get(keyField), splitterGenerator, partition);
				}
				else{
...
*/
	...
	protected static void cleanPartitioAndGetNextSplitterIfNecessary(IPartition partition, SplittersGenerator splittersGenerator, IBlock blockSPL, HashSet<IBlock> splittedBlocks, boolean blockOfSplitterHasBeenSplitted,boolean hasOnlyUnaryReactions, boolean updateTheSplitter) {
		...
		for (IBlock splittedBlock : splittedBlocks) {
			...
			//Otherwise, I have actually split the block. I remove it from the partition if it is empty.
				...
				if(hasOnlyUnaryReactions){
					if(CRNBisimulationsNAryWithoutListsOfReactions.USETOLERANCE){
						biggestSubBlock = splittedBlock.getBST(CRNBisimulationsNAryWithoutListsOfReactions.TOLERANCE).getBiggestSubBlock();
					}
					else{
	...
	private static boolean computeMeasuresUsedToSplitWithRespectToTheSplitter(
			Reduction red, ICRN crn, IBlock blockSPL, HashSet<ISpecies> splitterGenerators,IPartition partition, HashMap<IComposite, BigDecimal> multisetCoefficients, Integer iteration, HashMap<ICRNReaction, Integer> consideredAtIteration, ISpeciesCounterHandler[] speciesCounters,
		...
		for(ICRNReaction reaction : crn.getReactions()) {
			if(reaction.getRate().compareTo(BigDecimal.ZERO)!=0){
				...
				HashMap<ISpecies, Integer> net = reaction.computeProductsMinusReagentsHashMap();
				for(Entry<ISpecies, Integer> spAndMult : net.entrySet()) {
					ISpecies sp = spAndMult.getKey();
					if(blockSPL.getSpecies().contains(sp)) {
						anyReactionConsidered=true;
						BigDecimal netStocOfsplitterSpecies = BigDecimal.valueOf(spAndMult.getValue());
						BigDecimal val = reaction.getRate().multiply(netStocOfsplitterSpecies);
						if(reagents.isUnary()){	
							ISpecies reagent = reagents.getFirstReagent();
							ISpeciesCounterHandler currentSpeciesCounter =getOrAddSpeciesCounterHandler(speciesCounters, speciesCountersHM, reagent);
...
//					boolean anyReactionConsidered2 = computeNetProductionRateForGivenReactions(splitterGenerators, reactions.reactions, aSpeciesOfSplitter,hasOneLabelOnly,multisetCoefficients,speciesCounters,speciesCountersHM);
...
//	private static void updateNROfReagents(ICRNReaction reaction, HashSet<ISpecies> splitterGenerators, ISpecies splitterSpecies,boolean onlyOneLabel, HashMap<IComposite, BigDecimal> multisetCoefficients, int scale, RoundingMode rm, ISpeciesCounterHandler[] speciesCounters, HashMap<ISpecies,ISpeciesCounterHandler> speciesCountersHM) {
//
//		IComposite reagents = reaction.getReagents();
//		IComposite products = reaction.getProducts();
//		
...
//	}
	...
	protected static ISpeciesCounterHandler getOrAddSpeciesCounterHandler(ISpeciesCounterHandler[] speciesCounters,
			HashMap<ISpecies, ISpeciesCounterHandler> speciesCountersHM, ISpecies reagent) {
		return getOrAddSpeciesCounterHandler(speciesCounters, speciesCountersHM, reagent,true);
	}
	
	protected static ISpeciesCounterHandler getOrAddSpeciesCounterHandler(ISpeciesCounterHandler[] speciesCounters,
			HashMap<ISpecies, ISpeciesCounterHandler> speciesCountersHM, ISpecies reagent, boolean add) {
...
//	private static boolean computeNetProductionRateForGivenReactions(HashSet<ISpecies> splitterGenerators,Collection<ICRNReaction> consideredReactions,ISpecies aSpeciesOfSplitter, boolean hasOneLabelOnly, HashMap<IComposite, BigDecimal> multisetCoefficients, ISpeciesCounterHandler[] speciesCounters, HashMap<ISpecies, ISpeciesCounterHandler> speciesCountersHM) {
//		boolean anyReactionConsidered=false;
//		if(consideredReactions!=null){
//			anyReactionConsidered=true;
//			for (ICRNReaction reactionsWithNonZeroStoichiometry : consideredReactions) {
//				if(reactionsWithNonZeroStoichiometry.getRate().compareTo(BigDecimal.ZERO)!=0){
//					updateNROfReagents(reactionsWithNonZeroStoichiometry,splitterGenerators,aSpeciesOfSplitter,hasOneLabelOnly,multisetCoefficients,SCALE,RM,speciesCounters,speciesCountersHM);
//				}
//			}
//		}
//		return anyReactionConsidered;
...
*/
	...
	public static CRNandPartition computeReducedCRNQuotient(ICRN crn, String name, IPartition partition, List<String> symbolicParameters,List<IConstraint> constraints,List<String> parameters, String commentSymbol,MessageConsoleStream out, BufferedWriter bwOut,Terminator terminator) throws IOException {
		...
		for (ICRNReaction reaction : crn.getReactions()) {
			...
			if(BigDecimal.ZERO.compareTo(rate)!=0) {
				IComposite reducedReagents = CRNBisimulationsNAryWithoutListsOfReactions.getNewCompositeReplaceSpeciesWithReducedOneOfBlock(reaction.getReagents(),partition,correspondenceBlock_ReducedSpecies);
				IComposite reducedProducts = CRNBisimulationsNAryWithoutListsOfReactions.getNewCompositeReplaceSpeciesWithReducedOneOfBlock(reaction.getProducts(),partition,correspondenceBlock_ReducedSpecies);
	...
	private static void scaleRateOfReactionAccordingToProductOfSizesOfBlocksOfReagents(ICRNReaction originalReaction, ICRNReaction reducedReaction, IPartition partitionOnOriginalCRN){
		...
		if(productOfBlockSizesOfReagents!=1){
			if(rate!=null){
				rate = rate.divide(BigDecimal.valueOf(productOfBlockSizesOfReagents), CRNBisimulationsNAryWithoutListsOfReactions.SCALE,CRNBisimulationsNAryWithoutListsOfReactions.RM);
			}
			if(rate==null || CRNReducerCommandLine.KEEPTRACEOFRATEEXPRESSIONS){
				rateExpression = "("+rateExpression+")/"+productOfBlockSizesOfReagents;
			}
