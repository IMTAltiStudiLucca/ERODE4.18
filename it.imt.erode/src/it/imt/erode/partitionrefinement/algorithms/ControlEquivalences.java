...
public class ControlEquivalences {
	...
	public static IPartitionsAndBoolean computeCoarsestModifyingModelExpandingParams(Reduction red, ICRN crn, IPartition partition,
		...
		*/
		...
		if(!(red.equals(Reduction.UFE))){
			CRNReducerCommandLine.printWarning(out,bwOut,"Please invoke this method using UFE. I terminate.");
			return new IPartitionsAndBoolean(obtainedPartition, false,null);
		}
...
//		}
		...
		long begin = System.currentTimeMillis();
		CRNReducerCommandLine.println(out,bwOut,"\n\tExpanding the parameters so that each parameter appears in one reaction and creating the data structures for partitioning parameters... ");
		CRNReducerCommandLine.println(out,bwOut,"\tBeware: this functionality modifies the current model.");
		...
		//I know I'll get 1 parameter per reaction
		int numberOfParams = crn.getReactions().size();
		LinkedHashMap<String, ISpecies> paramToParamSpecies = new LinkedHashMap<>(numberOfParams);
		...
		IPartition partitionOfParams = new Partition(numberOfParams,-offsetOnIdOfParams);
		{
			//int id =0;
			...
			for(String param : crn.getParameters()) {
				...
				Integer blockId =  Integer.valueOf(constantStr);
				IBlock block = occurrenceToBlock.get(blockId);
				if(block==null) {
					block=new Block();
					partitionOfParams.add(block);
					occurrenceToBlock.put(blockId, block);
				}
			...
			*/
			...
			for(ICRNReaction reaction : crn.getReactions()) {
				...
				occ=occ+1;
				String paramName="c_"+currentId+"_"+occ;
				reaction.setRate(BigDecimal.valueOf(constant), paramName);
				//crn.addParameter(newParamName, String.valueOf(constant));
				//crn.getMath().setVariable(newParamName, constant);
				
				//Then I create the paramAsSpecies
				ISpecies paramAsSpecies =  new FakeSpeciesCompact(paramName,id);
				paramToParamSpecies.put(paramName, paramAsSpecies);
				id++;
				
				//Then I add it to the partition of params
				IBlock block = occurrenceToBlock.get(currentId);
				if(block==null) {
					block=new Block();
					partitionOfParams.add(block);
					occurrenceToBlock.put(currentId, block);
				}
				block.addSpecies(paramAsSpecies);
			}
		...
		CRNReducerCommandLine.print(out,bwOut,"\tCompleted: "+paramToParamSpecies.size()+ " parameters have been created and pre-partitioned in "+partitionOfParams.size()+" blocks... ");
...
//				}
		...
		*/
		...
		CRNReducerCommandLine.println(out,bwOut,"Time necessary: "+timeString+ " (s).");
		...
		ICRN curriedfG = new CRN(crn.getName(),crn.getMath(),out,bwOut);
		curriedfG.setExpectedNumberOfSpecies(crn.getSpecies().size()+crn.getReactions().size()/4);
...
//				}
				...
				addOneSpeciesPerRepresentativeParam(curriedfG, /*idToSpeciesCurriedFG,*/ partitionOfParams,crn.getSpecies().size());
			...
			//I have to change the ids of the paramAsSpecies in curriedfG
			//In the meantime I also do 1.3.2
			...
			int[] idsOfparamsWithChangedID=new int[partitionOfParams.size()];
			int offset = crn.getSpecies().size();
			for(int i=0;i<idsOfparamsWithChangedID.length;i++) {
				ISpecies sp =curriedfG.getSpecies().get(offset+i);
				idsOfparamsWithChangedID[i]=sp.getID();
				sp.setId(offset+i);
				
				IBlock currentfG = new Block();
				obtainedPartitionCurriedfG.add(currentfG);
				currentfG.addSpecies(sp);
			}
...
//				*/
//				ISpecies speciesfG = current.getRepresentative();
//				IBlock currentfG = new Block();
//				obtainedPartitionCurriedfG.add(currentfG);
...
//			}
			...
			CRNBisimulationsNAry.refine(Reduction.FE, curriedfG, curriedfG.getReactions(), obtainedPartitionCurriedfG/*, multisetCoefficients*/, null,speciesCountersHM, terminator,out,bwOut,true,null);
			...
			//I reset the ids of the parameter I changed
			for(int i=0;i<idsOfparamsWithChangedID.length;i++) {
				ISpecies paramAsSpecies = curriedfG.getSpecies().get(i+offset);
				
				//I also remove from obtainPartition the blocks correspoding to parameters
				IBlock blockOfParam = obtainedPartition.getBlockOf(paramAsSpecies);
				obtainedPartition.remove(blockOfParam,false);
				
				paramAsSpecies.setId(idsOfparamsWithChangedID[i]);
			}
...
//	}
	...
	*/
	...
	public static void/*HashMap<ISpecies, ISpecies>*/ addOneSpeciesPerRepresentativeParam(ICRN crnNew, /*ISpecies[] idToSpeciesCurriedFG,*/ IPartition partitionOfParams, int originalSpecies) throws IOException, UnsupportedFormatException {
		//int id=crnNew.getSpecies().size();
		int where=originalSpecies;
		//Transform the parameters to be perturbed in species
		
		//I add one species per representative param
		IBlock current = partitionOfParams.getFirstBlock();
		//Questa hashmap non mi serve piu'. Riusco direttamente la stessa specieasparam
		...
		while(current!=null) {
			...
			//Non ho bisogno di creare una nuova species. Posso usare paramAsSpecies. Ma devo fare partire gli id di paramaspecies da getCRN().getSpecies().size()
																//Attento, partition usa l'id delle specie per prendere il blocco a cui appartengono.
			//crnNew.addSpecies(paramAsSpecies);
			crnNew.setSpecies(paramAsSpecies,where);
			where++;
			/*
	...
	public static void copyReactionsReplacingParamsWithRepresentativeAndCurrying(/*HashMap<ISpecies, ISpecies> representativeOfBlockOfPartitionOfParamsToSpeciesFG,*/ICRN crn, ICRN crnNew, /*ISpecies[] idToSpeciesCurriedFG,*/ /*HashMap<String,ISpecies> speciesNameToSpeciesOfCurriedfG,*/ LinkedHashMap<String, ISpecies> paramToParamSpecies, IPartition partitionOfParams) throws IOException, UnsupportedFormatException {
		...
		for (ICRNReaction reaction : crn.getReactions()) {
			...
			if(paramToSp!=null) {
				...
				*/
				paramToSp = partitionOfParams.getBlockOf(paramToSp).getRepresentative();
			...
			if(where==crnNew.getReactions().size()) {
				CRNImporter.addReaction(crnNew, newReagents, newProducts, newReaction);
			}
			else{
				crnNew.getReactions().set(where, reaction);
			}
			where++;
		}
