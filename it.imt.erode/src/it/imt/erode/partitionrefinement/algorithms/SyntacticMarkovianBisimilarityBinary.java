...
//SyntacticMarkovianBisimilarityAllLabelsNoSelfLoopsViaNoSelfClass
public class SyntacticMarkovianBisimilarityBinary {
...
//		}
		...
		refineSMB(crnNew,obtainedPartition,compositesPartition,fakeLabels,speciesCounters,terminator,out,bwOut);
	...
	protected static ICRN addSelfLoops(ICRN crn, MessageConsoleStream out, BufferedWriter bwOut) throws IOException {
		CRNReducerCommandLine.print(out,bwOut," (adding self loops) ");
		ICRN crnNew = new CRN(crn.getName(),crn.getMath(),out,bwOut);
	
		CRN.copyParameters(crn, crnNew);
		ISpecies[] idToNewSpecies = CRN.copySpecies(crn, crnNew);
		
		HashMap<IComposite, BigDecimal> outGoingRates = new HashMap<IComposite, BigDecimal>(/*crn.getReagents().size()*/);
		CRN.copyReactions(crn, crnNew, idToNewSpecies, outGoingRates);
		for (Entry<IComposite, BigDecimal> pair : outGoingRates.entrySet()) {
			IComposite reagents = pair.getKey();
			BigDecimal minusTotalOut = BigDecimal.ZERO.subtract(pair.getValue()); 
			//IComposite products = crnNew.addProductIfNew(reagents);
			IComposite products = reagents;
			ICRNReaction totalOutReaction = new CRNReaction(minusTotalOut, reagents, products, minusTotalOut.toPlainString(),"minusTotalOut");
			CRNImporter.addReaction(crnNew, reagents, products, totalOutReaction);
		}
		return crnNew;
	}
	...
	private static void refineSMB(ICRN crn, IPartition partition, ICompositePartition compositesPartition, List<ILabel> fakeLabels, SpeciesCounterHandler[] speciesCounters, Terminator terminator, MessageConsoleStream out, BufferedWriter bwOut) {
	...
	private static void splitSMB(ICRN crn, IPartition partition, SplittersGeneratorSMB splittersGenerator, ICompositePartition compositesPartition, boolean refineUpBlocksAfterAnySplit, int iteration, SpeciesCounterHandler[] speciesCounters, HashMap<IComposite,ArrayList<ICRNReaction>> reactionsToConsiderForEachComposite) {
		...
		boolean anyReactionFound = computeConditionalRatesTowardsTheSplitterBlock(crn, /*labelSPL,*/ blockSPL,splitterGenerators,speciesCounters,hasOnlyUnaryReactions,reactionsToConsiderForEachComposite,partition);
	...
	public static void cleanPartitioAndGetNextSplitterIfNecessaryFocusingOnTheCurrentCompBlockOnly(IPartition partition, 
		...
		if(actuallysplit){
			switch (SyntacticMarkovianBisimilarityBinary.HOWTOSPLITCOMPOSITEBLOCKS) {
...
/*	protected static void cleanPartitioAndGetNextSplitterIfNecessary(IPartition partition, SplittersGeneratorMSB splittersGenerator, Set<IBlock> splittedBlocks, ICompositePartition compositesPartition, boolean refineUpBlocksAfterAnySplit) {
	...
	*/
	...
	private static boolean computeConditionalRatesTowardsTheSplitterBlock(ICRN crn, /*ILabel labelSPL,*/ ICompositeBlock blockSPL, HashSet<ISpecies> splitterGenerators, SpeciesCounterHandler[] speciesCounters, boolean hasOneLabelOnly, HashMap<IComposite,ArrayList<ICRNReaction>> reactionsToConsiderForEachComposite,IPartition partition) {
		...
		for (IComposite aCompositeOfSplitter : blockSPL.getComposites()) {
			...
			if(consideredReactions!=null){
				...
				for (ICRNReaction incomingReaction : consideredReactions) {
					//By adding this if we get SE rather than SMB
					//if(!multisetEquivalent(incomingReaction.getReagents(), incomingReaction.getProducts(), partition)) {
						if(incomingReaction.getRate().compareTo(BigDecimal.ZERO)!=0){
							increaseSMBCounterOfReagents(incomingReaction,/*labelSPL,*/ splitterGenerators,speciesCounters,hasOneLabelOnly);
						}
					//}
				}
			}
	...
	private static void increaseSMBCounterOfReagents(ICRNReaction reaction, /*ILabel labelSPL,*/HashSet<ISpecies> splitterGenerators, SpeciesCounterHandler[] speciesCounters, boolean onlyOneLabel) {
		IComposite reagents = reaction.getReagents();
