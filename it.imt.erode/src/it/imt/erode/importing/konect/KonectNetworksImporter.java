 ...
 */
public class KonectNetworksImporter extends AbstractImporter{

	private static final String zero = "0";
	private static final String one = "1";

	public KonectNetworksImporter(String fileName, MessageConsoleStream out,BufferedWriter bwOut,IMessageDialogShower msgDialogShower) {
		...
		while ((line = br.readLine()) != null) {
			...
			if(line.equals("")||line.startsWith("%")){
				...
				if(!added) {
					...
					IComposite targetComposite = new Composite(sourceSpecies, targetSpecies);
//					if(CRNReducerCommandLine.univoqueProducts){
//						////getCRN().addProduct(targetComposite);
//						//getCRN().addProductIfNew(targetComposite);
//					}
					
					ICRNReaction reaction = null;
					ICRNReaction reverseReaction = null;
					if(tokens.length==2) {
						reaction = new CRNReaction(BigDecimal.ONE, (IComposite)sourceSpecies, targetComposite, one,null);
						reverseReaction = new CRNReaction(BigDecimal.ONE, (IComposite)targetSpecies, targetComposite, one,null);
					}
					else {
						reaction = new CRNReaction(new BigDecimal(tokens[2]), (IComposite)sourceSpecies, targetComposite, tokens[2],null);
						reverseReaction = new CRNReaction(new BigDecimal(tokens[2]), (IComposite)targetSpecies, targetComposite,tokens[2],null);
					}
		...
		CRNReducerCommandLine.println(out,bwOut,"\t"+ignoredReactions + " reactions ignored because already added as reverse.");
	}
	...
	private ISpecies getOrAddSpecies(String nodeId, HashMap<String, ISpecies> nameToSpecies) {
		...
		if(species==null) {
			...
			//Every species is used as reagent. When I create a new species, I add it to the set of reagents and products.
