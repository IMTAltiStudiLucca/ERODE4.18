...
package it.imt.erode.expression.evaluator;
...
extends Object
{
	...
	private void validateName(String nam) {
		if(!Character.isLetter(nam.charAt(0))          ) { throw new IllegalArgumentException("Names for constants, variables and functions must start with a letter ("+nam+")"     ); }
		...
		for(ofs=beg; (ofs=skipWhitespace(expression,ofs,end))<=end; ofs++) {
			...
			EvaluateOperand: {
				...
				if(cur.unary!=LEFT_SIDE) {
					...
					if(ch0=='-') { nxt=getOperator(unary_minus); }                                              // unary '-': right-binding, high precedence operation (different from subtract)
				}
	...
	implements OperatorHandler, FunctionHandler
	{
		...
		// To add/remove operators change evaluateOperator() and registration
		//Andrea: doing 30.1*6.0 was resulting in 180.60000000000002. This was a problem for us. I therefore use BigDecimals to do operations.
		public double evaluateOperator(double lft, char opr, double rgt) {
			switch(opr) {
			case '=' : return rgt;                                                                  // simple assignment, used as the final operation, must be maximum precedence
			case '^' : return Math.pow(lft,rgt);                                                    // power
			case unary_minus : return -rgt;                                                                 // unary negation
			case '*' : return BigDecimal.valueOf(lft).multiply(BigDecimal.valueOf(rgt)).doubleValue();// multiply (classical)
			//case '×' : return BigDecimal.valueOf(lft).multiply(BigDecimal.valueOf(rgt)).doubleValue();// multiply (because it's a Unicode world out there)
			//case '·' : return BigDecimal.valueOf(lft).multiply(BigDecimal.valueOf(rgt)).doubleValue();// multiply (because it's a Unicode world out there)
			case '(' : return BigDecimal.valueOf(lft).multiply(BigDecimal.valueOf(rgt)).doubleValue();// multiply (implicit due to brackets, e.g "(a)(b)")
			//reaction.setRate(reaction.getRate().divide(bd,CRNBisimulationsNAry.getSCALE(),CRNBisimulationsNAry.RM), exp);
			case '/' : return BigDecimal.valueOf(lft).divide(BigDecimal.valueOf(rgt),/*CRNBisimulationsNAry.*/getSCALE(),/*CRNBisimulationsNAry.*/RM).doubleValue();// divide (classical computing)
			case '÷' : return BigDecimal.valueOf(lft).divide(BigDecimal.valueOf(rgt),/*CRNBisimulationsNAry.*/getSCALE(),/*CRNBisimulationsNAry.*/RM).doubleValue();// divide (because it's a Unicode world out there)
			case '%' : return BigDecimal.valueOf(lft).remainder(BigDecimal.valueOf(rgt)).doubleValue();// remainder
			//case '%' : return BigDecimal.valueOf(lft).remainder(BigDecimal.valueOf(rgt),new MathContext(CRNBisimulationsNAry.getSCALE(),CRNBisimulationsNAry.RM)).doubleValue();// remainder
			case '+' : return lft+rgt;                                                              // add/unary-positive
			case '-' : return lft-rgt;                                                              // subtract/unary-negative
			default  : throw new UnsupportedOperationException("MathEval internal operator setup is incorrect - internal operator \""+opr+"\" not handled");
			}
    ...
    static private final Operator       OPR_SUB =new Operator('-',20                    ,DefaultImpl.INSTANCE); // subtract/unary-negative
		...
		static private final Operator       OPR_EQU =new Operator('=',99,99,RIGHT_SIDE,true ,DefaultImpl.INSTANCE); // simple assignment, used as the final operation, must be maximum precedence
		static private final Operator       OPR_PWR =new Operator('^',80,81,NO_SIDE   ,false,DefaultImpl.INSTANCE); // power
		static private final Operator       OPR_NEG =new Operator(unary_minus,60,60,RIGHT_SIDE,true ,DefaultImpl.INSTANCE); // unary negation
		static private final Operator       OPR_MLT1=new Operator('*',40                    ,DefaultImpl.INSTANCE); // multiply (classical)
