...
import it.imt.erode.commandline.BooleanNetworkCommandLineRandomizedGFE;
...
public class EntryPointBNRandomizedFBE {

	private BooleanNetworkCommandLineRandomizedGFE erode;
	...
	public static void main(String[] args) {
		EntryPointBNRandomizedFBE entry = new EntryPointBNRandomizedFBE(true, true);
		
		String model="ap-1_else-0_wt.bnet";
		entry.loadBNet(model);

		try {
			entry.computeRandomizedFBEPartition();
		} catch (Z3Exception | UnsupportedFormatException | IOException e1) {
			e1.printStackTrace();
		}


		String init="INPUTS"; //OUTPUTS,INPUTSONEBLOCK,OUTPUTSONEBLOCK
		int[] initPartition=entry.buildPartition(init);
		try {
			entry.computeRandomizedFBEPartition(initPartition);
		} catch (Z3Exception | UnsupportedFormatException | IOException e1) {
			e1.printStackTrace();
		}
	...
	public int[] computeRandomizedFBEPartition() throws UnsupportedFormatException, Z3Exception, IOException{
		int[] initialPartitionArray=buildPartition("MAX");
		return computeRandomizedFBEPartition(initialPartitionArray,false);
	}
	
	public int[] computeRandomizedFBEPartition(int[] initialPartitionArray) throws UnsupportedFormatException, Z3Exception, IOException{
		//checkBNLoaded();
		return computeRandomizedFBEPartition(initialPartitionArray,false);
	}
	public int[] computeRandomizedFBEPartition(int[] initialPartitionArray, boolean numbersAreIDOfRepresentativeSpecies) throws UnsupportedFormatException, Z3Exception, IOException{
		checkBNLoaded();
		
		CRNReducerCommandLine.println(out,bwOut,"Computing Randomized FBE partition");
		...
		try {
			...
			IPartitionAndBoolean obtainedPartitionAndBool = erode.handleReduceCommand("reduceRandomizedFBE({aggregationFunction=>PLUS,computeOnlyPartition=>true,print=>false})",false,"rndfbe",out,bwOut);
			...
			CRNReducerCommandLine.println(out,bwOut,"RandomizedFBE partition computed");
		}finally {
	...
	public EntryPointBNRandomizedFBE(boolean printPartitions, boolean printModels) {
	...
	public int[] computeBBEPartition(String initialPartition) throws UnsupportedFormatException, Z3Exception, IOException{
		...
		if(initialPartition.equals("MAX")){
			checkBNLoaded();
			int[] initialPartitionArray=buildPartition(initialPartition);
	...
	private void completeLoadingBN(GuessPrepartitionBN guessPrep, String modelName,
		...
		erode = new BooleanNetworkCommandLineRandomizedGFE(new CommandsReader(new ArrayList<String>(0),out,bwOut),bn,defaultPartition,false);
		bnLoaded=true;
